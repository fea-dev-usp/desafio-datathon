...
#Temos respectivamente, os índices de shangai e hongkong
#além disso, temos o preço de fechamento do bitcoin
...
jacarta <- read.csv(file = "dado_jacarta.csv", fill = FALSE)
...
merg_jacarta_crypto <- merge(dado_jacarta, dado_crypto, join = "inner")
...
df_jacarta_sem <- transf_sem(merg_jacarta_crypto)
...
df_jacarta_sem$dado_jacarta <- get_return(df_jacarta_sem$dado_jacarta)
df_jacarta_sem$dado_crypto <- get_return(df_jacarta_sem$dado_crypto)
...
correlacao_jacarta <- cor(df_jacarta_sem)
...
#plot_hongkong_crypto <- df_hongkong_sem %>% ggplot(aes(x = dado_crypto, y = dado_hongkong)) +
#  geom_point()

#plot_shangai_crypto <- df_shangai_sem %>% ggplot(aes(x = dado_crypto, y = dado_shangai)) +
#  geom_point()
...
dados_jacarta_per <- dummy_filter(df_jacarta_sem$dado_jacarta, turn_pct = FALSE)
...
crise_jacarta <- escp(dados_jacarta_per)
...
crise_correlacao_jacarta <- crr_crise(crise_jacarta$PCT_change, df_jacarta_sem$dado_crypto)
...
#df_hongkong_sem[index(crise_hongkong)] %>% ggplot(aes(x = dado_hongkong, y = dado_crypto)) +
#  geom_point() 

#plotando shangai em crise
#df_shangai_sem[index(crise_shangai)] %>% ggplot(aes(x = dado_shangai, y = dado_crypto)) +
#  geom_point()
